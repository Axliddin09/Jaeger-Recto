...
const OrderItem = ({food}) => {
  ...
  const {foodItems, removeFromCart} = useContext(FoodContex)
  ...
  return (
    <li className="mb-6" key={food.id}>
                <div className="flex items-center mb-3">
                  <div className="w-[297px] flex justify-between items-center ">
                    <div className="flex items-center">
                      <img className="rounded-full h-11 w-11" src={food.img} alt="" />
                      <div className="ml-[5.39px] ">
                        <h5 className="text-sm font-medium leading-[18.2px] mb-1">{food.title.split(' ').splice(0, 4).join(' ')}</h5>
                        <p className="text-light text-xs font-medium leading-none">$ {food.price}</p>
                      </div>
                    </div>
                    <div className="py-3 px-5 border bg-base-form-bg mr-0.5 border-base-dark-line rounded-md">
                      {foodItems[food.id]}
                    </div>
                  </div>
                  <div className="ml-3">
                ...
                <div className="flex justify-between">
                  <input className="w-[295px] p-[14px] bg-base-form-bg border border-base-dark-line rounded-md" type="text" placeholder="Please, just a little bit spicy only."/>
                  <button className="w-14 h-14 flex justify-center items-center rounded-md border border-accents-red" 
